...
export function SigninForm({ className, ...props }: React.ComponentProps<"div">) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await Axios.post(SummaryApi.auth.Signin.url, { email, password });
      ...
      const userDetailsResponse = await Axios.get(SummaryApi.user.userDetails.url);
      dispatch(setUserDetails(userDetailsResponse.data));

      toast.success("Login successful!");
      
      // Check if we have a returnTo path in the location state
  ...
  return (
    <div className={`flex flex-col gap-6 ${className}`} {...props}>
      <Card className="overflow-hidden bg-gradient-to-r from-card to-input">
          ...
          <form className="p-6 md:p-8" onSubmit={handleLogin}>
            <div className="flex flex-col gap-6">
              ...
              <div className="grid grid-cols-3 gap-4">
                <Button variant="outline" className="w-full bg-gradient-to-tr from-card/50 to-input/70 ">
                ...
                <Button variant="outline" className="w-full bg-gradient-to-tl from-card/50 to-input/70 ">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="w-5 h-5">
                    <path
